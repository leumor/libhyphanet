...
static const uint32_t IT[4][256] = {
    {
...
#define LROUND(r)                                                              \
    s0 = W_[r * 4 + 0] ^ uint32_t(S[t0 >> 24]) << 24                           \
         ^ uint32_t(S[(t1 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(S[t3 & 0xff]);        \
    s1 = W_[r * 4 + 1] ^ uint32_t(S[t1 >> 24]) << 24                           \
         ^ uint32_t(S[(t2 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(S[t0 & 0xff]);        \
    s2 = W_[r * 4 + 2] ^ uint32_t(S[t2 >> 24]) << 24                           \
         ^ uint32_t(S[(t3 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(S[t1 & 0xff]);        \
...
#define ILROUND(r)                                                             \
    s0 = W_[r * 4 + 0] ^ uint32_t(IS[t0 >> 24]) << 24                          \
         ^ uint32_t(IS[(t3 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t1 & 0xff]);      \
    s1 = W_[r * 4 + 1] ^ uint32_t(IS[t1 >> 24]) << 24                          \
         ^ uint32_t(IS[(t0 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t2 & 0xff]);      \
    s2 = W_[r * 4 + 2] ^ uint32_t(IS[t2 >> 24]) << 24                          \
         ^ uint32_t(IS[(t1 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t3 & 0xff]);      \
                           ...
                           block_cipher::direction direction)
{
    ...
    for (int b = 0; b < 44; b++) printf("%08x ", W_[b]);
    printf("\n");
#endif
    return true;
}
...
{
    if (impl_) return impl_->encrypt_block(in, out);
...
{
    if (impl_) return impl_->decrypt_block(in, out);
...
void rijndael128_128::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);

    return block_cipher::decrypt_blocks(in, out, n);
}

void rijndael128_160::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael128_192::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael128_224::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael128_256::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael128_128::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);

    return block_cipher::encrypt_blocks(in, out, n);
}

void rijndael128_160::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael128_192::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael128_224::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael128_256::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void detail::rijndael256::encrypt_blocks(const unsigned char* in,
                                         unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void detail::rijndael256::decrypt_blocks(const unsigned char* in,
                                         unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void detail::rijndael224::encrypt_blocks(const unsigned char* in,
                                         unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void detail::rijndael224::decrypt_blocks(const unsigned char* in,
                                         unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael224_256::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael224_256::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael160_128::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael160_160::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael160_192::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael160_224::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael160_256::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael160_128::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael160_160::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael160_192::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael160_224::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael160_256::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael192_128::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael192_160::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael192_192::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael192_224::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael192_256::encrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->encrypt_blocks(in, out, n);
...
void rijndael192_128::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael192_160::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael192_192::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael192_224::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
void rijndael192_256::decrypt_blocks(const unsigned char* in,
                                     unsigned char* out, size_t n)
{
    if (impl_) return impl_->decrypt_blocks(in, out, n);
...
rijndael128_128::rijndael128_128(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael128_128_impl_aesni), 32);
...
#ifdef CPPCRYPTO_DEBUG
    for (int b = 0; b < 52; b++) printf("%08x ", W_[b]);
    printf("\n");
#endif
    return true;
}
...
rijndael128_192::rijndael128_192(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael128_192_impl_aesni), 32);
...
#ifdef CPPCRYPTO_DEBUG
    for (int b = 0; b < 60; b++) printf("%08x ", W_[b]);
    printf("\n");
#endif
    return true;
}
...
rijndael128_256::rijndael128_256(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael128_256_impl_aesni), 32);
...
rijndael128_224::rijndael128_224(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael128_224_impl_aesni), 32);
...
rijndael128_160::rijndael128_160(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael128_160_impl_aesni), 32);
...
#define LROUND256(r)                                                           \
    s0 = W_[r * 8 + 0] ^ uint32_t(S[t0 >> 24]) << 24                           \
         ^ uint32_t(S[(t1 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(S[t4 & 0xff]);        \
    s1 = W_[r * 8 + 1] ^ uint32_t(S[t1 >> 24]) << 24                           \
         ^ uint32_t(S[(t2 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t4 >> 8) & 0xff]) << 8 ^ uint32_t(S[t5 & 0xff]);        \
    s2 = W_[r * 8 + 2] ^ uint32_t(S[t2 >> 24]) << 24                           \
         ^ uint32_t(S[(t3 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t5 >> 8) & 0xff]) << 8 ^ uint32_t(S[t6 & 0xff]);        \
    s3 = W_[r * 8 + 3] ^ uint32_t(S[t3 >> 24]) << 24                           \
         ^ uint32_t(S[(t4 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t6 >> 8) & 0xff]) << 8 ^ uint32_t(S[t7 & 0xff]);        \
    s4 = W_[r * 8 + 4] ^ uint32_t(S[t4 >> 24]) << 24                           \
         ^ uint32_t(S[(t5 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t7 >> 8) & 0xff]) << 8 ^ uint32_t(S[t0 & 0xff]);        \
    s5 = W_[r * 8 + 5] ^ uint32_t(S[t5 >> 24]) << 24                           \
         ^ uint32_t(S[(t6 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(S[t1 & 0xff]);        \
    s6 = W_[r * 8 + 6] ^ uint32_t(S[t6 >> 24]) << 24                           \
         ^ uint32_t(S[(t7 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t1 >> 8) & 0xff]) << 8 ^ uint32_t(S[t2 & 0xff]);        \
...
#define ILROUND256(r)                                                          \
    s0 = W_[r * 8 + 0] ^ uint32_t(IS[t0 >> 24]) << 24                          \
         ^ uint32_t(IS[(t7 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t5 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t4 & 0xff]);      \
    s1 = W_[r * 8 + 1] ^ uint32_t(IS[t1 >> 24]) << 24                          \
         ^ uint32_t(IS[(t0 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t6 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t5 & 0xff]);      \
    s2 = W_[r * 8 + 2] ^ uint32_t(IS[t2 >> 24]) << 24                          \
         ^ uint32_t(IS[(t1 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t7 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t6 & 0xff]);      \
    s3 = W_[r * 8 + 3] ^ uint32_t(IS[t3 >> 24]) << 24                          \
         ^ uint32_t(IS[(t2 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t7 & 0xff]);      \
    s4 = W_[r * 8 + 4] ^ uint32_t(IS[t4 >> 24]) << 24                          \
         ^ uint32_t(IS[(t3 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t1 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t0 & 0xff]);      \
    s5 = W_[r * 8 + 5] ^ uint32_t(IS[t5 >> 24]) << 24                          \
         ^ uint32_t(IS[(t4 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t1 & 0xff]);      \
    s6 = W_[r * 8 + 6] ^ uint32_t(IS[t6 >> 24]) << 24                          \
...
rijndael256_128::rijndael256_128()
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael256_128_impl_aesni), 32);
...
rijndael256_224::rijndael256_224()
{
    ...
    if (cpu_info::aesni() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael256_224_impl_aesni), 32);
...
rijndael256_160::rijndael256_160()
{
    ...
    if (cpu_info::aesni() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael256_160_impl_aesni), 32);
...
rijndael256_192::rijndael256_192()
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael256_192_impl_aesni), 32);
...
#define LROUND192(r)                                                           \
    s0 = W_[r * 6 + 0] ^ uint32_t(S[t0 >> 24]) << 24                           \
         ^ uint32_t(S[(t1 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(S[t3 & 0xff]);        \
    s1 = W_[r * 6 + 1] ^ uint32_t(S[t1 >> 24]) << 24                           \
         ^ uint32_t(S[(t2 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(S[t4 & 0xff]);        \
    s2 = W_[r * 6 + 2] ^ uint32_t(S[t2 >> 24]) << 24                           \
         ^ uint32_t(S[(t3 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t4 >> 8) & 0xff]) << 8 ^ uint32_t(S[t5 & 0xff]);        \
    s3 = W_[r * 6 + 3] ^ uint32_t(S[t3 >> 24]) << 24                           \
         ^ uint32_t(S[(t4 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t5 >> 8) & 0xff]) << 8 ^ uint32_t(S[t0 & 0xff]);        \
    s4 = W_[r * 6 + 4] ^ uint32_t(S[t4 >> 24]) << 24                           \
         ^ uint32_t(S[(t5 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(S[t1 & 0xff]);        \
...
#define ILROUND192(r)                                                          \
    s0 = W_[r * 6 + 0] ^ uint32_t(IS[t0 >> 24]) << 24                          \
         ^ uint32_t(IS[(t5 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t4 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t3 & 0xff]);      \
    s1 = W_[r * 6 + 1] ^ uint32_t(IS[t1 >> 24]) << 24                          \
         ^ uint32_t(IS[(t0 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t5 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t4 & 0xff]);      \
    s2 = W_[r * 6 + 2] ^ uint32_t(IS[t2 >> 24]) << 24                          \
         ^ uint32_t(IS[(t1 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t5 & 0xff]);      \
    s3 = W_[r * 6 + 3] ^ uint32_t(IS[t3 >> 24]) << 24                          \
         ^ uint32_t(IS[(t2 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t1 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t0 & 0xff]);      \
    s4 = W_[r * 6 + 4] ^ uint32_t(IS[t4 >> 24]) << 24                          \
         ^ uint32_t(IS[(t3 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t1 & 0xff]);      \
...
rijndael192_128::rijndael192_128(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael192_128_impl_aesni), 32);
...
rijndael192_160::rijndael192_160(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael192_160_impl_aesni), 32);
...
rijndael192_192::rijndael192_192(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael192_192_impl_aesni), 32);
...
rijndael192_224::rijndael192_224(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael192_224_impl_aesni), 32);
...
rijndael192_256::rijndael192_256(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael192_256_impl_aesni), 32);
...
#define LROUND160(r)                                                           \
    s0 = W_[r * 5 + 0] ^ uint32_t(S[t0 >> 24]) << 24                           \
         ^ uint32_t(S[(t1 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(S[t3 & 0xff]);        \
    s1 = W_[r * 5 + 1] ^ uint32_t(S[t1 >> 24]) << 24                           \
         ^ uint32_t(S[(t2 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(S[t4 & 0xff]);        \
    s2 = W_[r * 5 + 2] ^ uint32_t(S[t2 >> 24]) << 24                           \
         ^ uint32_t(S[(t3 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t4 >> 8) & 0xff]) << 8 ^ uint32_t(S[t0 & 0xff]);        \
    s3 = W_[r * 5 + 3] ^ uint32_t(S[t3 >> 24]) << 24                           \
         ^ uint32_t(S[(t4 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(S[t1 & 0xff]);        \
...
#define ILROUND160(r)                                                          \
    s0 = W_[r * 5 + 0] ^ uint32_t(IS[t0 >> 24]) << 24                          \
         ^ uint32_t(IS[(t4 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t2 & 0xff]);      \
    s1 = W_[r * 5 + 1] ^ uint32_t(IS[t1 >> 24]) << 24                          \
         ^ uint32_t(IS[(t0 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t4 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t3 & 0xff]);      \
    s2 = W_[r * 5 + 2] ^ uint32_t(IS[t2 >> 24]) << 24                          \
         ^ uint32_t(IS[(t1 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t4 & 0xff]);      \
    s3 = W_[r * 5 + 3] ^ uint32_t(IS[t3 >> 24]) << 24                          \
         ^ uint32_t(IS[(t2 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t1 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t0 & 0xff]);      \
...
#define KEYSWAP160(Nr)                                                         \
    for (int i = 0; i < 5 * Nr / 2; i += 5) {                                  \
        std::swap(w[i + 0], w[5 * Nr + 0 - i]);                                \
        std::swap(w[i + 1], w[5 * Nr + 1 - i]);                                \
        std::swap(w[i + 2], w[5 * Nr + 2 - i]);                                \
        std::swap(w[i + 3], w[5 * Nr + 3 - i]);                                \
...
rijndael160_128::rijndael160_128(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael160_128_impl_aesni), 32);
...
rijndael160_160::rijndael160_160(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael160_160_impl_aesni), 32);
...
rijndael160_192::rijndael160_192(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael160_192_impl_aesni), 32);
...
rijndael160_224::rijndael160_224(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael160_224_impl_aesni), 32);
...
rijndael160_256::rijndael160_256(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael160_256_impl_aesni), 32);
...
#define LROUND224(r)                                                           \
    s0 = W_[r * 7 + 0] ^ uint32_t(S[t0 >> 24]) << 24                           \
         ^ uint32_t(S[(t1 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(S[t4 & 0xff]);        \
    s1 = W_[r * 7 + 1] ^ uint32_t(S[t1 >> 24]) << 24                           \
         ^ uint32_t(S[(t2 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(S[t5 & 0xff]);        \
    s2 = W_[r * 7 + 2] ^ uint32_t(S[t2 >> 24]) << 24                           \
         ^ uint32_t(S[(t3 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t4 >> 8) & 0xff]) << 8 ^ uint32_t(S[t6 & 0xff]);        \
    s3 = W_[r * 7 + 3] ^ uint32_t(S[t3 >> 24]) << 24                           \
         ^ uint32_t(S[(t4 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t5 >> 8) & 0xff]) << 8 ^ uint32_t(S[t0 & 0xff]);        \
    s4 = W_[r * 7 + 4] ^ uint32_t(S[t4 >> 24]) << 24                           \
         ^ uint32_t(S[(t5 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t6 >> 8) & 0xff]) << 8 ^ uint32_t(S[t1 & 0xff]);        \
    s5 = W_[r * 7 + 5] ^ uint32_t(S[t5 >> 24]) << 24                           \
         ^ uint32_t(S[(t6 >> 16) & 0xff]) << 16                                \
         ^ uint32_t(S[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(S[t2 & 0xff]);        \
...
#define ILROUND224(r)                                                          \
    s0 = W_[r * 7 + 0] ^ uint32_t(IS[t0 >> 24]) << 24                          \
         ^ uint32_t(IS[(t6 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t5 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t3 & 0xff]);      \
    s1 = W_[r * 7 + 1] ^ uint32_t(IS[t1 >> 24]) << 24                          \
         ^ uint32_t(IS[(t0 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t6 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t4 & 0xff]);      \
    s2 = W_[r * 7 + 2] ^ uint32_t(IS[t2 >> 24]) << 24                          \
         ^ uint32_t(IS[(t1 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t0 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t5 & 0xff]);      \
    s3 = W_[r * 7 + 3] ^ uint32_t(IS[t3 >> 24]) << 24                          \
         ^ uint32_t(IS[(t2 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t1 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t6 & 0xff]);      \
    s4 = W_[r * 7 + 4] ^ uint32_t(IS[t4 >> 24]) << 24                          \
         ^ uint32_t(IS[(t3 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t2 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t0 & 0xff]);      \
    s5 = W_[r * 7 + 5] ^ uint32_t(IS[t5 >> 24]) << 24                          \
         ^ uint32_t(IS[(t4 >> 16) & 0xff]) << 16                               \
         ^ uint32_t(IS[(t3 >> 8) & 0xff]) << 8 ^ uint32_t(IS[t1 & 0xff]);      \
...
#define KEYSWAP224(Nr)                                                         \
    for (int i = 0; i < 7 * Nr / 2; i += 7) {                                  \
        std::swap(w[i + 0], w[7 * Nr + 0 - i]);                                \
        std::swap(w[i + 1], w[7 * Nr + 1 - i]);                                \
        std::swap(w[i + 2], w[7 * Nr + 2 - i]);                                \
        std::swap(w[i + 3], w[7 * Nr + 3 - i]);                                \
        std::swap(w[i + 4], w[7 * Nr + 4 - i]);                                \
        std::swap(w[i + 5], w[7 * Nr + 5 - i]);                                \
...
namespace detail {
    rijndael224::rijndael224(): impl_(0) {}
...
rijndael224_128::rijndael224_128()
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael224_128_impl_aesni), 32);
...
rijndael224_160::rijndael224_160()
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael224_160_impl_aesni), 32);
...
rijndael224_224::rijndael224_224()
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael224_224_impl_aesni), 32);
...
rijndael224_192::rijndael224_192()
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael224_192_impl_aesni), 32);
...
rijndael224_256::rijndael224_256(): impl_(0)
{
    ...
    if (cpu_info::aesni() && cpu_info::sse41() && cpu_info::avx2()) {
        void* p
            = aligned_allocate(sizeof(detail::rijndael224_256_impl_aesni), 32);
