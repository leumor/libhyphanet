...
{
    t = static_cast<T*>(aligned_allocate(A, sizeof(T) * N));
}
...
template<typename T, size_t N, size_t A>
aligned_pod_array<T, N, A>::aligned_pod_array(
    aligned_pod_array<T, N, A>&& other)
{
    t = other.t;
    other.t = nullptr;
}

template<typename T, size_t N, size_t A> aligned_pod_array<T, N, A>&
aligned_pod_array<T, N, A>::operator=(aligned_pod_array<T, N, A>&& other)
{
    std::swap(t, other.t);
    return *this;
}
...
template<typename T, size_t N, size_t A>
aligned_pod_array<T, N, A>::aligned_pod_array(
    const aligned_pod_array<T, N, A>& other)
    : t(nullptr)
{
...
template<typename T, size_t N, size_t A> aligned_pod_array<T, N, A>&
aligned_pod_array<T, N, A>::operator=(const aligned_pod_array<T, N, A>& other)
{
...
public:
    ...
    template<typename RT, typename P> void create(const P& p);
...
template<typename T, size_t A> aligned_impl_ptr<T, A>::aligned_impl_ptr()
    : t(nullptr)
{}

template<typename T, size_t A> template<typename RT>
...
template<typename T, size_t A> template<typename RT, typename P>
...
template<typename T, size_t A> void aligned_impl_ptr<T, A>::destroy()
{
    if (t) {
        t->~T();
        aligned_deallocate(t);
...
template<typename T, size_t A> aligned_impl_ptr<T, A>&
aligned_impl_ptr<T, A>::operator=(aligned_impl_ptr<T, A>&& other)
{
    std::swap(t, other.t);
    return *this;
}
