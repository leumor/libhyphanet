 ...
 *
 * @details
 * Key is the base class for various types of user keys in the system. It
 * provides the common interface and functionalities for all derived key types.
 * The derived classes represent different key types and handle their specific
...
public:
     ...
     * cryptographic algorithm, and any associated meta strings. It is used
     * across various Key types to standardize the initialization and
     ...
     * cryptographic algorithm, and any associated meta strings. It is used to
     * create a Key object with specific cryptographic properties and associated
     * data.
     *
     * @param key A Key_params structure containing the initial parameters for
     * the Key object.
     *
     * @throws exception::Malformed_uri If any of the provided key parameters
     * are invalid, this constructor will invoke check_invariants() which may
     * throw an exception.
     */
     ...
     * return the URI representation of the Key object. The URI includes all
     * necessary information to identify and use the key, such as the routing
     * key, crypto key, and any meta strings or extra data associated with the
     * key.
     *
     * If the Key is an insertable key, the routing key of the URI is the
     * private key.
     *
     ...
     *
     * @details
     * A request URI is a URI that is used to request content from the network.
     * It does not contain the private key.
     *
     * If you want to give people access to content at an URI, you should always
     * publish only the request URI. Never give away the insert URI, this allows
     * anyone to insert under your URI!
     *
     * This pure virtual function must be implemented by derived classes to
     ...
     *
     * @details
     * This static method creates a unique pointer to a Key object based on the
     * type specified in the Uri. It can create different types of keys such as
     * USK, SSK, CHK, and KSK. The method also determines if the key is
     * insertable based on the extra data in the URI.
     *
     * It calls init_from_uri() of the derived class to initialize the key.
     *
     * @param uri The URI from which to create the Key object.
     *
     * @return A unique pointer to the created Key object.
     *
     * @throws exception::Malformed_uri If the URI is invalid or the key type is
     * unknown.
     */
    ...
    /**
     * @brief Returns the cryptographic algorithm used by the Key object.
     *
...
protected:
     ...
     *
     * @details
     * This method initializes the Key object using the routing key, crypto key,
     * and meta strings extracted from the given Uri. It checks the validity of
     * the URI and throws an exception if the URI is malformed.
     *
     * @param uri The Uri from which to initialize the Key object.
     *
     * @throws exception::Malformed_uri If the URI is invalid or the crypto key
     * is invalid.
     *
     * @sa Key::create()
     */
    ...
    /**
     * @brief Pops and returns the first meta string from the Key object.
     *
     * @details
     * This method removes the first meta string from the Key object's meta
     * strings and returns it. If there are no meta strings left, it returns
     * std::nullopt.
     *
     * @return An optional containing the first meta string if available, or
     * std::nullopt if there are no meta strings left.
     */
...
private:
     ...
     *
     * @details
     * This method checks the internal state of the Key object to ensure that
     * all required fields such as the routing key are properly set. It throws
     * an exception if any invariant is violated.
     *
     ...
     *
     * @details
     * A encryption key is a part of the Hyphanet URI (address) that is used
...
public:
     ...
     *
     * @details
     * This constructor initializes an Insertable object with a private key
     * provided by the user. The private key is used to insert new versions of
     * data into the network. It is essential for maintaining mutable content
     * where the owner wishes to update the content securely.
     *
     * @param priv_key The private key as a vector of bytes, which will be used
     * for data insertion.
     */
 ...
 *
 * @details
 * It's usually for sites that are going to change over time. For example, a
 * website that may need news to be updated or information to be corrected,
 * added or deleted.
 *
...
public:
     ...
     *
     * @details
     * This constructor initializes a Subspace_key object with the provided key
     * parameters, which include the routing key, cryptographic key,
     * cryptographic algorithm, and any associated meta strings. Additionally,
     * it sets the document name which is used to generate a routing key. The
     * Subspace_key is a subtype of Key that encodes a document name into it,
     * which will later be used to generate a routing key.
     *
     * @param key A Key_params structure containing the initial parameters for
     * the Subspace_key object.
     ...
     *
     * @details
     * Constructs a Uri that includes the routing key, crypto key, algorithm,
     * and document name. This URI can be used to retrieve or identify the
     * content associated with this Subspace Key.
     *
     * If the Key is an insertable key, the routing key of the URI is the
     * private key.
     *
    ...
    /**
     * @brief The fixed length of the extra data segment in the URI for a
     * Subspace_key object.
     *
     * @details
     * This constant defines the length of the extra data segment that is
     * appended to the URI of a Subspace_key. The extra data segment
     * contains additional information necessary for the Subspace_key, such as
     * the cryptographic algorithm used, control document flag, and compression
     * algorithm. The length is set to 5 bytes to accommodate these details.
     */
    static const size_t extra_length = 5;

    /**
     * @brief The standard length of the routing key used in the URI for a
     * Subspace_key object.
     *
     * @details
     * This constant specifies the length of the routing key part of the URI for
     * a Subspace_key. The routing key is a crucial component used in
     * the network layer to route requests to the appropriate node holding the
     * content. For Subspace Keys, the routing key length is fixed at 32 bytes
     * to ensure consistency and compatibility across the network.
     */
...
protected:
     ...
     *
     * @details
     * This method is used to obtain a vector of bytes that represent additional
     * data required for the Key's URI. This data typically includes information
     * such as the version of the key, flags indicating whether the key is for
     * insertion or retrieval, the cryptographic algorithm used, and other
     * key-specific metadata. The exact contents and format of the extra bytes
     * are determined by the specific type of Key and are used in constructing
     * the full URI for the Key.
     *
     * @return A vector of std::byte containing the extra data for the Key's
     * URI.
     */
...
/**
 * @brief Represents a **Signed Subspace %Key** (SSK) in the Hyphanet network.
 *
 * @details
 * SSKs are used for mutable content where updates are expected. They include a
 * public key for verification of content updates. SSKs are a fundamental part
...
public:
     ...
     * parameters, document name, and an optional public key.
     *
     * @details
     ...
     * cryptographic algorithm, and any associated meta strings. Additionally,
     * it sets the document name which is used to generate a routing key. The
     * Ssk is a subtype of Key that includes a public key for verification of
     * content updates. If a public key is provided, it is used to set the
     * public key of the Ssk; otherwise, the Ssk is initialized without a public
     * key.
     *
     * @param key A Key_params structure containing the initial parameters for
     * the Ssk object.
     ...
     * @param pub_key An optional vector of bytes representing the public key,
     ...
     * It does not contain the private key.
     *
     * If the Ssk is an Insertable_ssk, this method computes the request URI
     * which belongs to it. Otherwise it's identical to Ssk#to_uri().
     *
     * If you want to give people access to content at an URI, you should always
     * publish only the request URI. Never give away the insert URI, this allows
     * anyone to insert under your URI!
    ...
    /**
     * @brief Converts an Ssk (Signed Subspace Key) to a Usk (Updatable Subspace
     * Key) if possible.
     *
     * @details
     * This method attempts to convert the current Ssk object into a Usk object.
     * The conversion is based on parsing the document name of the Ssk to
     * extract a site name and an edition number. If the document name follows
     * the expected format that includes an edition number, a Usk object is
     * created with the same routing key, crypto key, cryptographic algorithm,
     * and meta strings as the Ssk, but with the addition of the extracted site
     * name and edition number.
     *
     * The method is useful for scenarios where mutable content is accessed
     * through Ssk but needs to be managed or referenced as Usk for updates or
     * versioning purposes.
     *
     * @return A std::optional containing a Usk object if the conversion is
     * successful, or an empty optional if the document name does not include an
     * edition number or does not follow the expected format.
     */
...
private:
    /**
     * @brief Calculates and stores the encrypted and hashed version of the
     * document name.
     *
     * @details
     * This method hashes the document name using SHA-256 and then encrypts the
     * hash using AES-256 with the cryptographic key of the Ssk. The result is
     * stored in the `encrypted_hashed_docname_` member variable. This encrypted
     * hash is used as part of the URI for the Ssk and is necessary for certain
     * network operations.
     */
    ...
    /**
     * @brief Stores the encrypted and hashed version of the document name.
     *
     * @details
     * This array holds the result of hashing the document name with SHA-256 and
     * then encrypting it using AES-256. It is used as part of the URI for the
     ...
     *
     * @details
     * This optional variable holds the public key if it is provided during the
     * construction of the Ssk. The public key is used for verifying the
     * authenticity of content updates in the network. If the public key is not
     * provided, this variable remains empty.
     */
    std::optional<std::vector<std::byte>> pub_key_;
};

/**
 * @brief An insertable version of an Ssk, containing a private key for data
 * insertion.
 *
 * @details
 * Insertable SSKs allow users to insert new versions of data into the Hyphanet
...
public:
     ...
     * document name, and private key.
     *
     * @details
     * This constructor initializes an Insertable_ssk object with the provided
     * key parameters, which include the routing key, cryptographic key,
     * cryptographic algorithm, and any associated meta strings. Additionally,
     * it sets the document name and the private key which are used to insert
     * new versions of data into the network. The Insertable_ssk is a subtype of
     * Ssk that allows for the insertion of new data versions.
     *
     * @param key A Key_params structure containing the initial parameters for
     * the Insertable_ssk object.
     * @param docname A string_view representing the document name to be
     * associated with the Insertable_ssk.
     * @param priv_key A vector of bytes representing the private key, used for
     * data insertion.
     */
    ...
    /**
     * @brief Constructs an Insertable_ssk object from an existing Ssk object
     * and a private key.
     *
     * @details
     * This constructor creates an Insertable_ssk object by copying the
     * properties of an existing Ssk object and setting the private key. It is
     * used when an Ssk already exists and needs to be made insertable by
     * providing the private key for data insertion.
     *
...
/**
 * @brief Represents an **Updatable Subspace %Key** (USK) for dynamic content
 * updates.
 *
 * @details
 * USKs facilitate a crude updating mechanism at the client level, allowing
 * users to request the latest version of a file without knowing the exact SSK.
 * This is particularly useful for dynamic content such as blogs or forums.
 *
 * Example:
 *
 * ```
...
public:
     ...
     * It does not contain the private key.
     *
     * If the Usk is an Insertable_usk, this method computes the request URI
     * which belongs to it. Otherwise it's identical to Usk#to_uri().
     *
     * If you want to give people access to content at an URI, you should always
     * publish only the request URI. Never give away the insert URI, this allows
     * anyone to insert under your URI!
    ...
    /**
     * @brief Converts a Usk (Updatable Subspace Key) to an Ssk (Signed Subspace
     * Key).
     *
     * @details
     * This method converts the current Usk object into an Ssk object. The
     * conversion involves creating a new Ssk object with the same routing key,
     * cryptographic key, cryptographic algorithm, and meta strings as the Usk,
     * but with the document name and edition number formatted according to the
     * Ssk's requirements.
     *
     * The Ssk created by this method represents a specific edition of the
     * content identified by the Usk, allowing for direct access to that
     * edition. This is particularly useful for retrieving or referencing a
     * specific version of mutable content within the network.
     *
     * @return An Ssk object representing a specific edition of the content
     * identified by the current Usk.
     */
...
private:
     ...
     * content on Hyphanet, such as blogs, forums, or wikis. It allows the
...
/**
 * @brief An insertable version of a Usk, containing a private key for data
 * insertion.
 *
 * @details
 * Similar to Insertable SSKs, Insertable USKs allow for the insertion of new
...
public:
     ...
     * properties of an existing Usk object and setting the private key. It is
     * used when an Usk already exists and needs to be made insertable by
     * providing the private key for data insertion. The Insertable_usk inherits
     * from both Usk and Insertable, combining the functionality of an Updatable
     * Subspace Key with the ability to insert new versions of data into the
     * network.
     *
...
/**
 * @brief Represents a **Keyword Signed %Key** (KSK) in the Hyphanet network.
 *
 * @details
...
/**
 * @brief Represents a **Content Hash %Key** (CHK) in the Hyphanet network.
 *
 * @details
 * CHks are used for immutable content. The key is derived from the content
 * itself, ensuring that any request for a CHK retrieves the exact content that
...
public:
     ...
     * algorithm.
     * @param control_document Flag indicating if the content is a control
     * document.
     ...
     *
     * @details
     * Constructs a URI that includes the routing key, crypto key, control
     * document flag, and compression algorithm. This URI can be used to
     * retrieve or identify the content associated with this CHK.
     *
     ...
     *
     * @details
     * Generates a node key based on the routing key. This key is used at the
     * node level to locate and retrieve the content.
     *
     ...
     * @details
     * This constant defines the length of the extra data segment that is
     * appended to the URI of a Content Hash Key (CHK). The extra data segment
     * contains additional information necessary for the CHK, such as the
     * cryptographic algorithm used, control document flag, and compression
     * algorithm. The length is set to 5 bytes to accommodate these details.
     */
    static const size_t extra_length = 5;

    /**
     * @brief The standard length of the routing key used in the URI for a Chk
     * object.
     *
     * @details
     * This constant specifies the length of the routing key part of the URI for
     * a Content Hash Key (CHK). The routing key is a crucial component used in
     * the network layer to route requests to the appropriate node holding the
     * content. For CHks, the routing key length is fixed at 32 bytes to ensure
     * consistency and compatibility across the network.
     */
...
private:
     ...
     * encryption algorithm, control document flag, and compression algorithm.
     *
     * @return A vector of bytes representing the extra data for the CHK URI.
     */
