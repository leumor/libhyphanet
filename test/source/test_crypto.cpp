...
chars_to_bytes(const std::array<signed char, 32>& chars)
{
    ...
    std::ranges::transform(chars, std::begin(bytes), [](char c) {
...
TEST_CASE("rijndael256_256", "[library][crypto]")
{
    ...
    auto pfcb_256_encrypt_iv = support::util::vector_to_array<std::byte, 32>(
        support::util::hex_to_bytes(
...
TEST_CASE("DSA", "[library][crypto]")
{
    ...
    fmt::println(
        "Private key MPI bytes: {:02x}", fmt::join(priv_key_mpi_bytes, " ")
    );
...
TEST_CASE(
    "Boost::multiprecision::mpz_int to CryptoPP::Integer conversion",
    "[library][crypto]"
)
{
