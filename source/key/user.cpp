...
{
    if (Key::get_routing_key().size() != routing_key_size) {
...
void Ssk::set_pub_key(const std::vector<std::byte>& pub_key)
{
    ...
    if (auto routing_key = Key::get_routing_key(); !routing_key.empty()) {
...
Uri Ssk::to_uri() const
{
    ...
    uri.append_meta_strings(Key::get_meta_strings());
    return uri;
}
...
std::unique_ptr<key::user::Usk> Ssk::to_usk() const
{
    if (auto sitename_edition = parse_sitename_edition(); sitename_edition) {
        ...
        return std::make_unique<Usk>(params, sitename, edition);
    }
    return nullptr;
}

std::shared_ptr<node::Ssk> Ssk::get_node_key() const
{
    // TODO Cache node key
    return std::make_shared<node::Ssk>(
        Key::get_routing_key(),
        encrypted_hashed_docname_,
...
void Insertable_ssk::init_from_uri(const Uri& uri)
{
    ...
    set_priv_key(Key::get_routing_key());
    set_routing_key(std::vector<std::byte>{}); // Disable pub_key/priv_key
                                               // matching check

    auto pub_key = crypto::dsa::make_pub_key(Insertable::get_priv_key());
    set_pub_key(pub_key);
...
Uri Insertable_ssk::to_uri() const
{
    ...
    uri.set_routing_key(Insertable::get_priv_key());
...
std::shared_ptr<node::Chk> Chk::get_node_key() const
{
    // TODO Cache node key
    return std::make_shared<node::Chk>(
