...
namespace impl {
    ...
    std::unique_ptr<Key> Key::create(const Uri& uri)
    {
        ...
        if (auto extra = uri.get_extra(); !extra.empty() && extra.size() >= 5
                                          && extra.at(1) == std::byte{1}) {
            is_insertable = true;
        }
    ...
    void Subspace_key::init_from_uri(const Uri& uri)
    {
        ...
        impl::Key::get_routing_key().reserve(routing_key_size);
    }

    void Subspace_key::check_invariants() const
    {
        if (impl::Key::get_routing_key().size() != routing_key_size) {
    ...
    std::vector<std::byte> Subspace_key::get_extra_bytes() const
    {
        std::vector<std::byte> extra_bytes{
            ...
            std::byte{static_cast<std::byte>(
                impl::Key::get_crypto_algorithm())}, // Crypto algorithm
            std::byte{static_cast<std::byte>(
                1 >> 8)}, // TODO: KeyBlock.HASH_SHA256 >> 8
    ...
    {
        Uri_params params;
        params.routing_key = impl::Key::get_routing_key();
        params.crypto_key = impl::Key::get_crypto_key();
    ...
    {
        impl::Subspace_key::init_from_uri(uri);
    ...
    {
        auto uri = impl::Subspace_key::to_uri();
        uri.set_uri_type(Uri_type::ssk);
        uri.append_meta_strings(impl::Key::get_meta_strings());
        return uri;
    }
    ...
    Ssk::parse_sitename_edition() const
    {
        static const std::regex docname_with_edition_re{
            fmt::format("(.*)\\{}([0-9]+)", separator)};
        std::smatch match;

        if (auto docname = impl::Subspace_key::get_docname();
    ...
    std::unique_ptr<key::user::Usk> Ssk::to_usk() const
    {
        if (auto sitename_edition = parse_sitename_edition();
            ...
            params.routing_key = impl::Key::get_routing_key();
            params.crypto_key = impl::Key::get_crypto_key();
            params.crypto_algorithm = impl::Key::get_crypto_algorithm();
            params.meta_strings = impl::Key::get_meta_strings();

            auto ptr = std::make_unique<Usk>(params, sitename, edition);
    ...
    Uri Insertable_ssk::to_uri() const
    {
        ...
        uri.set_routing_key(impl::Insertable::get_priv_key());
    ...
    {
        auto bytes = impl::Subspace_key::get_extra_bytes();
    ...
    void Chk::parse_compressor(std::byte byte_1, std::byte byte_2)
    {
        ...
        if (!util::in_range(compressor_value,
                            compressor::valid_compressor_types)) {
            throw exception::Malformed_uri{
                "Invalid URI: invalid extra data (compressor)"};
        }

        compressor_
            = static_cast<compressor::Compressor_type>(compressor_value);
    }
